...
export default function SettingsLayout({ children }) {
  ...
  return (
    ...
    >
      ...
      <main className="relative flex max-h-screen min-w-0 flex-col w-full overflow-hidden">
        <div className="relative z-10 flex min-h-0 flex-1 px-3 py-4 sm:px-6 sm:py-5">
          <div className="flex min-h-0 flex-1 flex-col rounded-3xl border border-(--border) surface-panel overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              <button
                onClick={() => setSidebarOpen(true)}
                className="md:hidden p-2 text-slate-700 absolute top-3 left-2 dark:text-white"
              >
                <Menu size={20} />
              </button>
              {children}
